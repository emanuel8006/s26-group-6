...
menu_cache: TTLCache = TTLCache(maxsize=100, ttl=3600) # Cache with a time-to-live of 60 minutes
...
async def get_menu(location_name: str, period_name: str) -> dict[str, Any]:
    ...
    Args:
         location_name (str): The name of the location to get the menu for.
         period_name (str): The name of the period to get the menu for (e.g., Breakfast, Lunch, Dinner).
    Returns:
    ...
    """
    # Import here to avoid cirvular imports between this file and location_service.py
    from backend.app.services.location_service import get_location_id, get_period_id_by_name

    location_id = get_location_id(location_name=location_name) # Location name -> Location ID
    if not location_id:
        return {
            "error": f"Unrecognized location name: {location_name}",
            "known_locations": ["stetson east", "international village"]
        }
    period_id = await get_period_id_by_name(location_id, period_name) # Period name -> Period ID
    if not period_id:
        return {
            "error": f"Period name {period_name} not found for today at {location_name}",
            "hint": "The menu may not have been published yet, or the period name may be mispelled."
        }
    ...
    # Parsing the stations and menu items from API response
