...
function PreferencesStep({ answers, toggleArr, set, onBack, onNext, step, isNewPlan }) {
  return (
    <div>
      ...
      <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '0.78rem', letterSpacing: '0.03em', color: '#9CA3AF', margin: '-4px 0 8px' }}>Based on vendors near Northeastern</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '1rem' }}>
        {CUISINE_OPTIONS.map(opt => <Chip key={opt} label={opt} selected={answers.cuisines.includes(opt)} onToggle={() => toggleArr('cuisines', opt)} />)}
      </div>

      <span style={st.sectionLabel}>WHAT DO YOU USUALLY EAT?</span>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '1rem' }}>
      ...
      <span style={st.sectionLabel}>DINING STYLE</span>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '1rem' }}>
