  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(4px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem' }} onClick={onClose}>
      <div style={{ background:'#fff',border:'2.5px solid #1a1a1a',borderRadius:'14px',padding:'1.8rem',maxWidth:'400px',width:'100%',boxShadow:'5px 6px 0 #1a1a1a' }} onClick={e=>e.stopPropagation()}>
        <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.72rem',letterSpacing:'0.12em',color:'#9CA3AF',margin:'0 0 4px' }}>LOG A SWIPE</p>
        <p style={{ fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:'1.4rem',color:'#1a1a1a',margin:'0 0 1.4rem' }}>Use a Swipe</p>
        <label style={{ display:'block',fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.7rem',letterSpacing:'0.1em',color:'#9CA3AF',marginBottom:'6px' }}>DINING LOCATION</label>
      ...
      <div style={st.hero}>
        ...
        <div style={st.heroInner}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'auto 1fr',gap:'2.5rem',alignItems:'center',marginTop:'2rem' }}>
            <div>
              <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.68rem',letterSpacing:'0.12em',color:'rgba(255,255,255,0.35)',margin:'0 0 4px' }}>SWIPES REMAINING</p>
              {totalSwipes === null ? (
              ...
              <div style={{ display:'flex',alignItems:'center',gap:'10px',flexWrap:'wrap' }}>
                {totalSwipes && <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.72rem',letterSpacing:'0.06em',color:'rgba(255,255,255,0.35)',margin:0 }}>OF {totalSwipes} · {pct}% REMAINING</p>}
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'8px' }}>
            {weekByDay.map(({ day, isToday, isPast, isFuture, count }) => (
              <div key={day} style={{
                ...
                <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.65rem',letterSpacing:'0.08em',color: isToday ? '#D42B2B' : '#9CA3AF',margin:'0 0 6px' }}>
                  {day}{isToday ? ' ·' : ''}
                </p>
                {isPast || isToday ? (
                  <>
            ...
            <div style={st.card}>
              ...
              {swipeLog.length === 0 ? (
                <div style={{ border:'2px dashed rgba(0,0,0,0.1)',borderRadius:'8px',padding:'2rem',textAlign:'center' }}>
                  <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.72rem',letterSpacing:'0.1em',color:'#9CA3AF',margin:'0 0 4px' }}>NO SWIPES LOGGED YET</p>
                    ...
                    return (
                      <div key={s.id} style={{ display:'flex',alignItems:'center',gap:'12px',padding:'11px 0',borderBottom: i < Math.min(swipeLog.length,10)-1 ? '1px solid rgba(0,0,0,0.05)' : 'none' }}>
                        <div style={{ width:'32px',height:'32px',borderRadius:'8px',background:'#FBF2D8',border:'1.5px solid rgba(0,0,0,0.08)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
                          <span style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.6rem',letterSpacing:'0.04em',color:'#1a1a1a' }}>{v?.short}</span>
                        </div>
                        <div style={{ flex:1,minWidth:0 }}>
                          <p style={{ fontFamily:"'Inter',sans-serif",fontWeight:500,fontSize:'0.88rem',color:'#1a1a1a',margin:'0 0 1px' }}>{v?.label}</p>
                          <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.65rem',letterSpacing:'0.05em',color:'#9CA3AF',margin:0 }}>{s.date} · {s.time}</p>
                        </div>
                        <span style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.72rem',letterSpacing:'0.06em',color:'#D42B2B',flexShrink:0 }}>−1 SWIPE</span>
                      </div>
                    )
                  })}
                </div>
            ...
            <div style={st.card}>
              ...
              {totalSwipes === null ? (
                <div style={{ padding:'1rem 0' }}>
                  ...
                  <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.72rem',letterSpacing:'0.06em',color:'#9CA3AF',margin:0 }}>NO SWIPE LIMIT — USE FREELY</p>
                </div>
              ) : (
                [
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:'8px' }}>
                {[
                  ...
                  <div key={venue} style={{ background:'#FAFAFA',border:'1.5px solid rgba(0,0,0,0.07)',borderRadius:'8px',padding:'10px 12px',display:'flex',gap:'10px',alignItems:'flex-start' }}>
                    ...
                    <div>
                      <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.72rem',letterSpacing:'0.06em',color:'#1a1a1a',margin:'0 0 2px' }}>{venue.toUpperCase()}</p>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:'8px' }}>
                {[
                  ...
                  <div key={title} style={{ background:bg,border:`1.5px solid ${border}`,borderRadius:'8px',padding:'10px 12px' }}>
                    <p style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'0.7rem',letterSpacing:'0.06em',color:'#1a1a1a',margin:'0 0 3px' }}>{title.toUpperCase()}</p>
